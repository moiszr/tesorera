    ...
    <div className="entra-hoja">
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,340px)_minmax(0,1fr)]">
        ...
        <div className="hoja overflow-hidden">
          <div className="flex items-baseline justify-between gap-3 border-b border-linea px-5 py-3.5">
          ...
          {pagos.length === 0 ? (
            <div className="px-5 py-8 text-center">
              <p className="text-tinta2">
                Su primer abono aparecerá aquí, con la fecha y la forma de pago.
              </p>
            </div>
          ) : (
            <ul>
              {pagos.map((p, i) => (
                <li key={p.id} style={{ ['--i' as string]: Math.min(i, 10) }} className="entra-renglon renglon">
                  <div className="flex min-h-[54px] items-center gap-3 px-5 py-2">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className={`truncate ${p.anulado ? 'text-tinta3' : ''}`}>
                        {fechaLarga(p.fecha)}
                        <span className="text-tinta3"> · {fechaRelativa(p.fecha)}</span>
                      </p>
                      <p className="truncate text-menuda text-tinta2">
                        {p.anulado === 1
                          ? `Anulado${p.nota_anulacion ? `: ${p.nota_anulacion}` : ' sin nota'}`
                          : p.nota
                            ? `${NOMBRE_METODO[p.metodo] ?? 'Otro'} · “${p.nota}”`
                            : (NOMBRE_METODO[p.metodo] ?? 'Otro')}
                      </p>
                    </div>

                    {p.anulado === 1 && (
                      <span className="shrink-0 rounded-full bg-[var(--sinpagos-fondo)] px-2.5 py-1 text-menuda font-medium text-[var(--sinpagos-tinta)]">
                        Anulado
                      </span>
                    )}
