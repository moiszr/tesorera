...
export function Dialogo({
  ...
  return (
    <dialog
      ref={ref}
      className="dialogo m-auto w-[calc(100vw-2rem)] rounded-hoja bg-hoja p-0 text-tinta shadow-dialogo backdrop:bg-[rgba(40,28,18,0.42)]"
    ...
    >
      <div className="flex items-start justify-between gap-4 px-5 pb-1 pt-5">
        ...
        <button
          ...
          onClick={alCerrar}
          aria-label="Cerrar"
          className="-mr-1.5 -mt-1.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-pieza text-tinta2 transition-colors hover:bg-[rgba(36,31,27,0.06)] hover:text-tinta"
        >
          <IconoCerrar tam={18} />
        </button>
      </div>
      <div className="px-5 pb-5 pt-2">{children}</div>
      {pie && <div className="flex flex-wrap justify-end gap-2 border-t border-linea bg-hoja2 px-5 py-4">{pie}</div>}
    </dialog>
  )
}
